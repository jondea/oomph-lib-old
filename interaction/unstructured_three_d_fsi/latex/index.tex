...
\textcolor{comment}{} \hyperlink{classMySolidTetgenMesh}{MySolidTetgenMesh}(\textcolor{keyword}{const} std::string& node\_file\_name,
  ...
  \{
   ...
   \textcolor{comment}{// Setup boundary coordinates for all boundaries}
   \textcolor{keywordtype}{char} filename[100];
   ofstream some\_file;
   \textcolor{keywordtype}{unsigned} nb=this->nboundary();
   \textcolor{keywordflow}{for} (\textcolor{keywordtype}{unsigned} b=0;b<nb;b++)
    \{
     sprintf(filename,\textcolor{stringliteral}{"RESLT/solid\_boundary\_test%i.dat"},b);
     some\_file.open(filename);
     this->\textcolor{keyword}{template} setup\_boundary\_coordinates<ELEMENT>(b,some\_file);
     some\_file.close();
    \}
...
\begin{DoxyCodeInclude}
   ...
   \textcolor{comment}{// Setup boundary coordinates for boundary.}
   \textcolor{comment}{// To be consistent with the boundary coordinates generated}
   \textcolor{comment}{// in the solid, we switch the direction of the normal.}
   \textcolor{comment}{// (Both meshes are generated from the same polygonal facets}
   \textcolor{comment}{// at the FSI interface).}
   \textcolor{keywordtype}{bool} switch\_normal=\textcolor{keyword}{true};
   ...
   \textcolor{keywordflow}{for} (\textcolor{keywordtype}{unsigned} b=0;b<nb;b++) 
    \{
     sprintf(filename,\textcolor{stringliteral}{"RESLT/fluid\_boundary\_test%i.dat"},b);
     some\_file.open(filename);
     this->\textcolor{keyword}{template} setup\_boundary\_coordinates<ELEMENT>(b,switch\_normal,some\_file);
     some\_file.close();
    \}
...
\textcolor{comment}{}
\textcolor{comment}{ /// Empty Destructor}
\textcolor{comment}{} \textcolor{keyword}{virtual} ~\hyperlink{classFluidTetMesh}{FluidTetMesh}() \{ \}
...
\begin{DoxyCodeInclude}
 ...
 \textcolor{comment}{// pointers to the meshes. }
 n=nsolid\_fsi\_boundary();
 \textcolor{keywordflow}{for} (\textcolor{keywordtype}{unsigned} i=0;i<n;i++)
  \{
   \textcolor{comment}{// Sanity check: Doc boundary coordinates from solid side}
   doc\_solid\_boundary\_coordinates(i);
   
   \textcolor{comment}{//Doc boundary coordinates in fluid}
   \textcolor{keywordtype}{char} filename[100];
   sprintf(filename,\textcolor{stringliteral}{"RESLT/fluid\_boundary\_coordinates%i.dat"},i);
   Multi\_domain\_functions::Doc\_boundary\_coordinate\_file.open(filename);
   
   \textcolor{comment}{// Setup FSI: Pass ID of fluid FSI boundary and associated}
   \textcolor{comment}{// mesh of solid fsi traction elements.}
   FSI\_functions::setup\_fluid\_load\_info\_for\_solid\_elements<FLUID\_ELEMENT,3>
    (\textcolor{keyword}{this},Fluid\_fsi\_boundary\_id[i],Fluid\_mesh\_pt,Solid\_fsi\_traction\_mesh\_pt[i]);
...
\{ 
 ...
 \textcolor{comment}{// Output solid boundaries}
 \textcolor{comment}{//------------------------}
 sprintf(filename,\textcolor{stringliteral}{"%s/solid\_boundaries%i.dat"},doc\_info.directory().c\_str(),
         doc\_info.number());
 some\_file.open(filename);
 ...
 \textcolor{comment}{// Output fluid boundaries}
 \textcolor{comment}{//------------------------}
 sprintf(filename,\textcolor{stringliteral}{"%s/fluid\_boundaries%i.dat"},doc\_info.directory().c\_str(),
         doc\_info.number());
 some\_file.open(filename);
 Fluid\_mesh\_pt->output\_boundaries(some\_file);
 some\_file.close();
 ...
 \textcolor{comment}{// Output fsi traction}
 \textcolor{comment}{//--------------------}
 sprintf(filename,\textcolor{stringliteral}{"%s/fsi\_traction%i.dat"},doc\_info.directory().c\_str(),
         doc\_info.number());
 some\_file.open(filename);
 \textcolor{keywordtype}{unsigned} n=nsolid\_fsi\_boundary();
 \textcolor{keywordflow}{for} (\textcolor{keywordtype}{unsigned} i=0;i<n;i++)
  \{
   Solid\_fsi\_traction\_mesh\_pt[i]->output(some\_file,npts);
  \}
 some\_file.close();

\} \textcolor{comment}{// end\_of\_doc}
