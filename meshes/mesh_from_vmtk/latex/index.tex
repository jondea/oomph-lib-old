The \href{http://www.vmtk.org}{\tt Vascular Modeling Toolkit} is a collection of libraries and tools for the 3D reconstruction, geometric analysis, mesh generation and surface data analysis for image-\/based modelling of blood vessels.
...
Before starting, make sure that you have V\+M\+TK installed on your machine and that you have your D\+I\+C\+OM (Digital Imaging and Communications in Medicine) images. Note that this tutorial describes the use of V\+M\+TK version 0.\+8.
...
You can extract a volume of interest (V\+OI) from a dataset with \+:
...
When satisfied with the V\+OI, press {\ttfamily q} or {\ttfamily e}. The new volume is displayed in the render window.
...
When the V\+OI is displayed in a render window, press {\ttfamily q} or {\ttfamily e} to proceed.
...
For typical vessels, a passband of 0.\+1 and a number of iteration of 30 should be OK. For example, for the surface model obtained in the section \hyperlink{index_surface}{The 3D surface reconstruction of a vascular segment} above, the resulting surface is\+:
...
\item The flag {\ttfamily -\/interactive} was set to {\ttfamily 0}\+: This means that {\ttfamily vmtkflowextensions} will not prompt the user about what inlet or outlet to extend, but it will perform the task on all the available open boundaries. If you use {\ttfamily -\/interactive} {\ttfamily 1}, V\+M\+TK prompts the user to specify which boundaries to extend through a graphical window.
\end{DoxyItemize}
...
\doxyfigcaption{The fluid surface with multiple boundary I\+Ds }
\end{DoxyImage}
...
\doxyfigcaption{The solid surface with multiple boundary I\+Ds for a wall thickness of 2mm }
\end{DoxyImage}
