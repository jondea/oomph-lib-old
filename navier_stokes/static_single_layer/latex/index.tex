...
\textcolor{comment}{/// Perform a parameter study. Pass name of output directory as }
\textcolor{comment}{}\textcolor{comment}{/// a string}
\textcolor{comment}{}\textcolor{comment}{//======================================================================}
\textcolor{keyword}{template}<\textcolor{keyword}{class} ELEMENT>
\textcolor{keywordtype}{void} \hyperlink{classPseudoSolidCapProblem_ae86ecaf62fa0920f5d14bf82a7f83b0e}{PseudoSolidCapProblem<ELEMENT>::parameter\_study}(\textcolor{keyword}{const} \textcolor{keywordtype}{
      string}& dir\_name)
\{
 \textcolor{comment}{// Create DocInfo object (allows checking if output directory exists)}
 DocInfo doc\_info;
 doc\_info.set\_directory(dir\_name);
 doc\_info.number()=0;
...
\textcolor{comment}{}\textcolor{comment}{//========================================================================}
\textcolor{keyword}{template}<\textcolor{keyword}{class} ELEMENT>
\textcolor{keywordtype}{void} \hyperlink{classPseudoSolidCapProblem_a2fb98a37bde5742cbdf3c91cb1f5eb2f}{PseudoSolidCapProblem<ELEMENT>::doc\_solution}(DocInfo& 
      doc\_info)
\{ 
 \textcolor{comment}{//Output stream}
 ofstream some\_file;
 \textcolor{keywordtype}{char} filename[100];
 ...
 \textcolor{comment}{//Output domain}
 ...
 \textcolor{comment}{// Number of interface elements}
 \textcolor{keywordtype}{unsigned} ninterface=Free\_surface\_mesh\_pt->nelement();
 \textcolor{comment}{//Find number of nodes in the last interface element}
 \textcolor{keywordtype}{unsigned} np = Free\_surface\_mesh\_pt->finite\_element\_pt(ninterface-1)->nnode();
 \textcolor{comment}{// Document the contact angle (in degrees), the height of the interface at }
 \textcolor{comment}{// the centre of the container, the height at the wall, the computed }
 \textcolor{comment}{// pressure drop across the interface and }
 \textcolor{comment}{// the analytic prediction of the pressure drop.}
 Trace\_file << Angle*180.0/MathematicalConstants::Pi;
 Trace\_file << \textcolor{stringliteral}{" "}  << Free\_surface\_mesh\_pt->finite\_element\_pt(0)
  ->node\_pt(0)->x(1)
            << \textcolor{stringliteral}{" "} 
            << Free\_surface\_mesh\_pt->finite\_element\_pt(ninterface-1)
  ->node\_pt(np-1)->x(1);
 Trace\_file << \textcolor{stringliteral}{" "} 
            << \textcolor{keyword}{dynamic\_cast<}ELEMENT*\textcolor{keyword}{>}(Bulk\_mesh\_pt->element\_pt(0))->p\_nst(0)-
               External\_pressure\_data\_pt->value(0);
 Trace\_file << \textcolor{stringliteral}{" "} << -2.0*cos(Angle)/Ca;
